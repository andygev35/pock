...
internal class AppController: NSResponder {
    ...
    /// Reload — notification-compatible wrapper (NotificationCenter passes Notification, not Bool)
    ...
    @objc internal func reload(shouldFetchLatestVersions: Bool) {
        func _reload() {
            tearDownTouchBar()
            // Use async dispatch instead of blocking sleep to avoid freezing the main thread
    ...
    @objc internal func relaunch() {
        guard let relaunchPath = Bundle.main.path(forResource: "Relaunch", ofType: nil) else {
            return
        }
        let task = Process()
        task.launchPath = relaunchPath
        task.arguments  = ["\(ProcessInfo.processInfo.processIdentifier)"]
        do {
            try task.run()
        } catch {
            Roger.error("[AppController]: Failed to launch Relaunch helper: \(error.localizedDescription)")
        }
    }
    ...
    private func registerForInternalNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(reloadFromNotification(_:)), name: .shouldReloadPock, object: nil)
